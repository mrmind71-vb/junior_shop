...
With grid0
    ...
    If bPrice Then
        cFields = cFields & "," & _
                myiif("(type = '6')" & cWhereField, "([OUT] * FILE1_11_ALL.PRICE * (1 - FILE1_11_ALL.DISCOUNT_RATE) * (1 + FILE1_11_ALL.TAX_RATE))")
    End If
    ...
    'cWhere = "(file1_10.ISNOITEM = 0 AND file1_10.ISNOITEM2 = 0)"
...
With grid2
    ...
    If bPrice Then
        cField = cField & "," & _
                myiif_r("(type = '6')" & cWhereField, "([OUT] * FILE1_11_ALL.PRICE * (1 - FILE1_11_ALL.DISCOUNT_RATE) * (1 + FILE1_11_ALL.TAX_RATE))")
    End If
...
With GRID7
    ...
    If .Rows > 1 Then
        If Val(.TextMatrix(1, ColNet)) <> 0 Then
            .TextMatrix(1, colRate) = Round((Val(.TextMatrix(1, ColSales)) / Val(.TextMatrix(1, ColNet))) * 100, 2)
        End If
        .Cell(flexcpAlignment, 1, 0, .Rows - 1, .Cols - 1) = 7
