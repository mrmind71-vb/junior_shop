...
Dim cStr1 As String
cF1 = ",(SELECT SUM(CASH) FROM FILE6_20H  WHERE FILE6_20H.STORE = VW_FILE6_20.STORE AND FILE6_20H.DATE >= " & DateSq(xDate1.text) & " AND FILE6_20H.DATE <= " & DateSq(xdate2.text) & " )"
cF2 = ",(SELECT SUM(VISA) FROM FILE6_20H  WHERE FILE6_20H.STORE = VW_FILE6_20.STORE AND FILE6_20H.DATE >= " & DateSq(xDate1.text) & " AND FILE6_20H.DATE <= " & DateSq(xdate2.text) & " )"
cF3 = ",(SELECT SUM(POINT) FROM FILE6_20H  WHERE FILE6_20H.STORE = VW_FILE6_20.STORE AND FILE6_20H.DATE >= " & DateSq(xDate1.text) & " AND FILE6_20H.DATE <= " & DateSq(xdate2.text) & " )"
cF4 = ",(SELECT SUM(LATE) FROM FILE6_20H  WHERE FILE6_20H.STORE = VW_FILE6_20.STORE AND FILE6_20H.DATE >= " & DateSq(xDate1.text) & " AND FILE6_20H.DATE <= " & DateSq(xdate2.text) & " )"

cF5 = myiif("FILE1_10.OUTLET = 1", "(VW_FILE6_20.QUANT)")
cF6 = myiif("FILE1_10.OUTLET = 1", "(VW_FILE6_20.TOTAL * ((100-VW_FILE6_20.RATE )/100))")

cStr1 = " SELECT VW_FILE6_20.store," & _
            "STORE_BR.DESCA," & _
            ...
            " INNER JOIN STORE_BR ON VW_FILE6_20.store = STORE_BR.CODE " & _
            " WHERE VW_FILE6_20.STORE <> '000' "
If pSec <> "" Then
    cStr1 = cStr1 & " AND [SECTION] = " & Val(pSec)
End If
If CGROUP <> "" Then cStr1 = cStr1 & " AND VW_FILE6_20.store  IN ( SELECT CODE FROM STORE_BR2 WHERE BRANCH_GR = " & MyParn(CGROUP) & " ) "
cStr1 = cStr1 & " AND DATE >= " & DateSq(xDate1.text)
cStr1 = cStr1 & " AND DATE <= " & DateSq(xdate2.text)
If Not bOpt5 Then cStr1 = cStr1 & " AND [store] IN (SELECT STORE FROM USERSHOP WHERE CODE = " & nusercode & " ) "
cStr1 = cStr1 & " GROUP BY VW_FILE6_20.store," & _
                " STORE_BR.DESCA " & _
                " ORDER BY VW_FILE6_20.store  "
...
Sub MYLOAD2(Optional cStore As String, Optional cCode As String, Optional cSec As String, Optional CGROUP As String)
    ...
    cString = cString & " AND DATE <= " & DateSq(xdate2.text)
    If Not bOpt5 Then
        cString = cString & " AND [store] IN (SELECT STORE FROM USERSHOP WHERE CODE = " & nusercode & " ) "
    End If
    
    If cStore <> "" Then
        cString = cString & " AND STORE = " & MyParn(cStore)
    End If
    
    If cCode <> "" Then
        cString = cString & " AND  FILE1_10.CODE = " & MyParn(cCode)
    End If
    If cSec <> "" Then
        cString = cString & " AND FILE1_10.[SECTION]= " & MyParn(cSec)
    End If
    
    If CGROUP <> "" Then
        cStr1 = cStr1 & " AND VW_FILE6_20.store  IN ( SELECT CODE FROM STORE_BR2 WHERE BRANCH_GR = " & MyParn(CGROUP) & " ) "
    End If
    
    cString = cString & " GROUP BY FILE1_10.[SECTION]," & _
                        " FILE1_10SC.DESCA," & _
                        " FILE1_10.[MOSM]" & _
                        " ORDER BY FILE1_10.[SECTION]," & _
                        " FILE1_10.[MOSM]"
...
Dim cString As String
cF4 = myiif("FILE1_10.Outlet = 1", "(VW_FILE6_20.QUANT)")
cF5 = myiif("FILE1_10.Outlet = 1", "(VW_FILE6_20.TOTAL_NET)")

cString = " SELECT FILE1_10.[SECTION]," & _
            " FILE1_10SC.DESCA , " & _
            " SUM(VW_FILE6_20.QUANT) AS t_q," & _
            " SUM(VW_FILE6_20.TOTAL_NET) AS t_sales , 0 , " & cF4 & " , " & cF5 & _
            " FROM VW_FILE6_20 " & _
            " INNER JOIN FILE1_10 ON VW_FILE6_20.ITEM = FILE1_10.ITEM" & _
            " INNER JOIN FILE1_10SC ON FILE1_10.[SECTION] = FILE1_10SC.CODE" & _
            " WHERE VW_FILE6_20.STORE <> '000' "
...
If cCode <> "" Then cString = cString & turn(cString) & " FILE1_10.CODE = " & MyParn(cCode)
If CGROUP <> "" Then cStr1 = cStr1 & " AND VW_FILE6_20.store  IN ( SELECT CODE FROM STORE_BR2 WHERE BRANCH_GR = " & MyParn(CGROUP) & " ) "

cString = cString & " GROUP BY FILE1_10.[SECTION]," & _
                    " FILE1_10SC.DESCA" & _
                    " ORDER BY FILE1_10.[SECTION] "
...
cString = cString & " AND DATE <= " & DateSq(xdate2.text)
If cStore <> "" Then cString = cString & turn(cString) & "STORE = " & MyParn(cStore)
If cSec <> "" Then cString = cString & turn(cString) & " FILE1_10.[SECTION]  = " & MyParn(cSec)
If CGROUP <> "" Then cStr1 = cStr1 & " AND VW_FILE6_20.store  IN ( SELECT CODE FROM STORE_BR2 WHERE BRANCH_GR = " & MyParn(CGROUP) & " ) "
cString = cString & " GROUP BY FILE1_10.[CODE]," & _
                    " FILE4_10.DESCA ORDER BY FILE1_10.[CODE] "
Set DATA3.Recordset = myRecordSet(cString, con)
Fixgrd3
End Sub
Sub MYLOAD4()
Dim cString As String
cString = " SELECT BRANCH_GR.[CODE]," & _
            " BRANCH_GR.DESCA , " & _
            " SUM(VW_FILE6_20.QUANT) AS t_q," & _
            " SUM(VW_FILE6_20.TOTAL_NET) AS t_sales " & _
            " FROM VW_FILE6_20 " & _
            " INNER JOIN STORE_BR2 ON VW_FILE6_20.[store] = STORE_BR2.CODE" & _
            " LEFT JOIN BRANCH_GR ON BRANCH_GR.CODE = STORE_BR2.BRANCH_GR" & _
            " WHERE VW_FILE6_20.STORE <> '000' "
If Not bOpt5 Then cString = cString & " AND [store] IN (SELECT STORE FROM USERSHOP WHERE CODE = " & nusercode & " ) "
cString = cString & turn(cString) & "DATE >= " & DateSq(xDate1.text)
cString = cString & turn(cString) & "DATE <= " & DateSq(xdate2.text)
