...
cFiled1 = ",(SELECT  SUM(TOTAL_NET)   FROM T_SALESDOC_ALL WHERE T_SALESDOC_ALL.STORE =  FILE6_80.BRANCH AND DATE >= " & DateSq(xDate1.text) & " AND DATE <= " & DateSq(xDate2.text) & " )"
cFiled2 = ",(SELECT  SUM(TOTAL_NET)   FROM T_SALESDOC_ALL WHERE T_SALESDOC_ALL.STORE =  FILE6_80.BRANCH AND DATE >= " & DateSq(dDate11) & " AND DATE <= " & DateSq(dDate22) & " )"
...
''''                " ,(SELECT  SUM(TOTAL_NET)   FROM T_SALESDOC_ALL INNER JOIN STORE_BR2 AS STORE_BR2_2 ON STORE_BR2_2.CODE = T_SALESDOC_ALL.STORE WHERE STORE_BR2_2.BRANCH_GR =  STORE_BR2.BRANCH_GR AND DATE >= " & DateSq(xDate1.text) & "  AND DATE <= " & DateSq(xdate2.text) & ")" & _
...
''''    data3.RecordSource = cString
    ...
    cString = " SELECT FILE6_80.GR_MAN , BRANCH_GR.DESCA, SUM(FILE6_80.value_TARGET  ) " & _
                " ,(SELECT  SUM(TOTAL_NET)   FROM T_SALESDOC_ALL INNER JOIN FILE6_80 AS FILE6_80_2 ON FILE6_80_2.BRANCH = T_SALESDOC_ALL.STORE  WHERE  FILE6_80_2.GR_MAN =  FILE6_80.GR_MAN  AND FILE6_80_2.DOC_NO = " & MyParn(xDoc_No.text) & "  AND DATE >= " & DateSq(xDate1.text) & "  AND DATE <= " & DateSq(xDate2.text) & ")" & _
                " FROM FILE6_80 INNER JOIN STORE_BR2 ON FILE6_80.BRANCH = STORE_BR2.CODE INNER JOIN BRANCH_GR ON BRANCH_GR.CODE = FILE6_80.GR_MAN  where FILE6_80.doc_no = " & MyParn(xDoc_No.text)
    If Not bOpt5 Then cString = cString & " AND FILE6_80.[BRANCH] IN (SELECT STORE FROM USERSHOP WHERE CODE = " & nusercode & " ) "
