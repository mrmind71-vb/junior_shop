...
If Not (loctable.EOF) Then
    If IsNull(loctable!maxOf) Then
        IncDataRs = ""
    ElseIf bNum Then
        IncDataRs = Val(loctable!maxOf & "") + 1
    Else
        IncDataRs = IncrementString(loctable!maxOf & "")
    End If
End If
loctable.Close
Set loctable = Nothing
...
End Function
Function DateShort(ByVal pDate As Variant) As String
If Not IsDate(pDate) Then
    DateShort = pDate
    Exit Function
End If
DateShort = MyParn(myFormat(pDate))
End Function
...
Public Function IncrementSuffix(ByVal strInput As String) As String
    ...
    For i = Len(strInput) To 1 Step -1
        If Not IsNumeric(Mid(strInput, i, 1)) Then Exit For
    Next i
    
    ' Split string into text prefix and numeric suffix
    strPrefix = Left(strInput, i)
    strNumber = Mid(strInput, i + 1)
    
    If strNumber <> "" Then
        ' Increment the number and maintain leading zeros
        lngNewNum = CLng(strNumber) + 1
        IncrementSuffix = strPrefix & Format(lngNewNum, String(Len(strNumber), "0"))
    Else
        ' If no number exists, append "1"
        IncrementSuffix = strInput & "1"
    End If
End Function
